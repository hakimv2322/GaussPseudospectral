...
%--------------------------------------------------------------------------
function Plots_Gauss_CMG3(q,w,delta,u,Det,p,method)
    %----------------------------------------------------------------------
...
%     longert = [longt; p.tf];
    ...
    plot(longt,delta(:,1),'.','markersize',14,'color',mycolor(2,:)); hold on
    plot(longt,delta(:,2),'.','markersize',14,'color',mycolor(4,:)); hold on
    plot(longt,delta(:,3),'.','markersize',14,'color',mycolor(6,:)); hold on
    
    if strcmp(method,'Pseudospectral')
        plot(T,d1l,'-','color',mycolor(2,:)); hold on
        plot(T,d2l,'-','color',mycolor(4,:)); hold on
        plot(T,d3l,'-','color',mycolor(6,:)); hold on
    end
...
%     end
    ...
    mytitle = ['Gimbal Angle Rates']; % title with latex
    myxlabel = '$t$ (sec.)'; % x label with latex
    myylabel = 'Rates (rad/sec)'; % y label with latex
    mylegend = {'$\frac{d}{dt}\delta_1$', '$\frac{d}{dt}\delta_2$', ...
        '$\frac{d}{dt}\delta_3$'}; % legend with latex
    xlabel(myxlabel) % create x label
    ylabel(myylabel) % create y label
    ylim([-3, 3])
    ha = gca; % get current axis handle
    ...
    myxlabel = '$t$ (sec.)'; % x label with latex
    myylabel = 'Determinant'; % y label with latex
    xlabel(myxlabel) % create x label
    ylabel(myylabel) % create y label
    ha = gca; % get current axis handle
    try
