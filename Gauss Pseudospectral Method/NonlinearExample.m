...
p.y10 = 2; p.y1f = 1; % boundary conditions
...
x = fmincon(@(x) 0,x0,[],[],[],[],[],[],@(x) constraintsWeird(x,p),options);
x0(p.y1i) = x(p.y1i); x0(p.ui) = x(p.ui);
...
% plots
Plots_Gauss_NonLin(y1,u,Lam,p,'Pseudospectral')
...
function [c,ceq] = constraints(x,p)
    ...
    F = ((p.tf-p.t0)/2)*(2*y1(2:end) + 2*u.*sqrt(y1(2:end)));
